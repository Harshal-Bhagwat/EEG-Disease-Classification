...
def build_cnn_model(input_shape):
    ...
    X = Conv2D(10, (2, 2), strides=(1, 1), name='conv0')(X)
    X = BatchNormalization(axis=3, name='bn0')(X)
    X = Activation('relu')(X)  # shape=(?, 238, 238, 32)
    X = MaxPooling2D((4, 4), name='max_pool0')(X)  # shape=(?, 59, 59, 32)

    # Flatten the CNN output
    X = Flatten()(X)  # shape=(?, 10816)

    # Fully Connected Layer
    X = Dense(128, activation='relu', name='fc1')(X)  # shape=(?, 128)
...
def build_cnn_model(input_shape):
    ...
    # CNN Block
    X = ZeroPadding2D((8, 8))(X_input)  # Adding excessive padding
    X = Conv2D(2, (20, 20), strides=(5, 5), activation='linear', name='conv0')(X)  # Large kernel, poor activation
    X = Flatten()(X)  # Flattening too early, losing spatial information

    # Fully Connected Layer
    X = Dense(1, activation='linear', name='fc1')(X)  # Very few neurons
    X = Dense(1, activation='sigmoid', name='fc2')(X)  # Output layer
...
cnn_model.fit(x=X_train, y=y_train, batch_size=32, epochs=2, validation_data=(X_val, y_val), callbacks=[tensorboard, checkpoint])
...
cnn_model.fit(x=X_train, y=y_train, batch_size=32, epochs=3, validation_data=(X_val, y_val), callbacks=[tensorboard, checkpoint])
...
cnn_model.fit(x=X_train, y=y_train, batch_size=32, epochs=1, validation_data=(X_val, y_val), callbacks=[tensorboard, checkpoint])
...
#best_model = load_model('models/cnn-parameters-improvement-23-0.91.keras')
