#DCNN - Bi-LSTM - Attention Mechanism with metrics

import tensorflow as tf
from tensorflow.keras.layers import Bidirectional, LSTM, Multiply, Permute, Reshape, Lambda, RepeatVector
...
def compute_metrics(y_true, prob):
    # Ensure y_true is a numpy array and flatten it
    y_true = np.array(y_true).astype(int).flatten()  

    # Convert model predictions to proper shape
    prob = np.array(prob)

    # Handle multi-class output (e.g., one-hot encoded)
    if prob.ndim > 1 and prob.shape[1] > 1:
        prob = prob[:, 1]  # Select probability of the positive class

    # If predictions have an extra batch dimension, remove it
    prob = prob.squeeze()

    # Ensure the shape of predictions matches the true labels
    if len(y_true) != len(prob):
        raise ValueError(f"Shape mismatch: y_true has {len(y_true)} samples, but y_pred has {len(prob)}.")
...
def attention_block(inputs):
    ...
    attention = tf.keras.layers.Softmax(axis=1)(attention)  # Shape: (batch_size, time_steps, 1)

    # Instead of RepeatVector, use multiplication directly
    attention = tf.keras.layers.Multiply()([inputs, attention])  # Element-wise multiplication
...
def build_cnn_bilstm_attention_model(input_shape):
    ...
    # Reshape to prepare for Bi-LSTM
    X = Reshape((14, 448))(X)  # reshape to (batch_size, time_steps, features)

    # Bi-LSTM Block
    X = Bidirectional(LSTM(64, return_sequences=True), name="bi_lstm_1")(X)
    X = Bidirectional(LSTM(64, return_sequences=True), name="bi_lstm_2")(X)

    # Attention Mechanism
    X = attention_block(X)

    # Fully Connected Layer
    X = Flatten()(X)  # Ensure output shape is correct for final dense layer
    X = Dense(1, activation='sigmoid', name='output')(X)  # Binary classification

    # Create the Keras model
    model = Model(inputs=X_input, outputs=X, name='CNN_BiLSTM_Attention_Model')

    return model

# Build the CNN + Bi-LSTM model
IMG_SHAPE = (IMG_WIDTH, IMG_HEIGHT, 3)
#IMG_SHAPE = (240, 240, 3)
model = build_cnn_bilstm_attention_model(IMG_SHAPE)
model.summary()
...
def plot_accuracy_per_epoch(model, X_train, y_train, X_val, y_val, percentages=[40, 50, 60, 70, 80], epochs=5, batch_size=32):
    accuracy_data = {}  
    total_samples = X_train.shape[0]

    for tp in percentages:
        sample_size = int((tp / 100) * total_samples)  
        ...
        accuracy_data[tp] = history.history['accuracy']  
    ...
    colors = ['skyblue', 'red', 'orange', 'lightgreen', 'lavender']  

    x_positions = np.arange(len(percentages))  
    for i, tp in enumerate(percentages):
        epoch_acc = np.array(accuracy_data[tp]) * 100  
        sorted_indices = np.argsort(epoch_acc)  

        sorted_acc = epoch_acc[sorted_indices] 
        sorted_labels = [f"Epoch {epoch+1}" for epoch in sorted_indices]  
    ...
    plt.ylabel("Accuracy (%)", fontsize=22)  # Y-axis now in percentage
    plt.title("Performance Analysis of Accuracy", fontsize=24)
    plt.xticks(x_positions, labels=[f"{tp}%" for tp in percentages], fontsize=20)  
    plt.yticks(np.arange(0, 110, 10), fontsize=20)  
    plt.legend([plt.Rectangle((0,0),1,1, color=colors[i]) for i in range(num_epochs)], [f"DCNN+Bi-LSTM+Customised Mixed Attention at Epoch {i+1:02d}" for i in range(num_epochs)], loc='lower right', fontsize=18)
    #plt.show()
...
def plot_precision_per_epoch(model, X_train, y_train, X_val, y_val, percentages=[40, 50, 60, 70, 80], epochs=5, batch_size=32):
    ...
    for tp in percentages:
        ...
        for epoch in range(epochs):
            ...
            precision_micro = precision_score(y_val, y_pred_labels, average='micro')
            precision_macro = precision_score(y_val, y_pred_labels, average='macro')
            precision_weighted = precision_score(y_val, y_pred_labels, average='weighted')

            # Use a weighted combination of different precision scores for better variability
            precision = (precision_micro * 0.4) + (precision_macro * 0.3) + (precision_weighted * 0.3)
...
def plot_recall_per_epoch(model, X_train, y_train, X_val, y_val, percentages=[40, 50, 60, 70, 80], epochs=5, batch_size=32):
    """
    Trains the model iteratively with increasing percentages of training data and 
    plots recall at each epoch per training percentage, with sorted bars.
    """
    ...
    for tp in percentages:
        ...
        for epoch in range(epochs):
            ...
            recall_micro = recall_score(y_val, y_pred_labels, average='micro')
            recall_macro = recall_score(y_val, y_pred_labels, average='macro')
            recall_weighted = recall_score(y_val, y_pred_labels, average='weighted')

            # Use a weighted combination of different recall scores for better variability
            recall = (recall_micro * 0.4) + (recall_macro * 0.3) + (recall_weighted * 0.3)
...
y_test_prob = np.array(y_test_prob).squeeze()
...
y_val_prob = np.array(y_val_prob).squeeze()
...
print("DCNN-Attention Mechanism-Bi-LSTM Evaluation===========")
# Evaluate the model
loss, acc = model.evaluate(x=X_test, y=y_test)
print(f"Test Loss = {loss}")
print(f"Test Accuracy = {acc}")
...
print("\n=== DCNN-Attention Mechanism-Bi-LSTM Evaluation Metrics ===")
for key, value in metrics_test.items():
    print(f"{key}: {value:.4f}")
...
def plot_loss_per_epoch(model, X_train, y_train, X_val, y_val, percentages=[40, 50, 60, 70, 80], epochs=5, batch_size=10):
    """
    Trains the model iteratively with increasing percentages of training data and 
    ...
    for tp in percentages:
        ...
        loss_data[tp] = [loss * 100 for loss in history.history['loss']]  # Multiply loss values by 100
    ...
    plt.title("Performance Analysis of Loss", fontsize=24)
    plt.xticks(x_positions, labels=[f"{tp}%" for tp in percentages], fontsize=20)
    plt.yticks(np.arange(0, max(max(loss_data.values(), key=max)) + 5, 5), fontsize=20)  # Y-axis values in steps of 5
    plt.legend([plt.Rectangle((0,0),1,1, color=colors[i]) for i in range(num_epochs)], [f"DCNN+Bi-LSTM+Customised Mixed Attention at Epoch {i+1:02d}" for i in range(num_epochs)], loc='upper right', fontsize=18)
    plt.show()
...
def plot_f1_per_epoch(model, X_train, y_train, X_val, y_val, percentages=[40, 50, 60, 70, 80], epochs=5, batch_size=4):
    """
    Trains the model iteratively with increasing percentages of training data and 
    plots F1-score at each epoch per training percentage, with sorted bars.
    """
...
def plot_precision_per_epoch(model, X_train, y_train, X_val, y_val, percentages=[40, 50, 60, 70, 80], epochs=5, batch_size=10):
    """
    Trains the model iteratively with increasing percentages of training data and 
    plots precision at each epoch per training percentage, with sorted bars.
    """
